...
final class CrudController {
     ...
     */
    public CrudController(String loaderoApiToken) {
        ClientUtils.checkArgumentsForNull(loaderoApiToken);
        
        this.loaderoApiToken = loaderoApiToken;
        this.client = new CustomHttpClient(loaderoApiToken);
    }
    ...
    public LoaderoModel get(String url, LoaderoModelType type) {
        ...
        try (CloseableHttpResponse res = client.build().execute(get)) {
            if (res.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                ...
                result = ClientUtils.httpEntityToModel(entity, type);
    ...
    public LoaderoModel update(String url, LoaderoModelType type, LoaderoModel newModel) {
        ClientUtils.checkArgumentsForNull(url, type, newModel);
    
        LoaderoModel result = null;
        String methodName = "";
        try {
            String modelToJson = ClientUtils.modelToJson(newModel);
            ...
            if (res.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
                result = ClientUtils.httpEntityToModel(
                        res.getEntity(),
                        type);
    ...
    public LoaderoModel post(String url, LoaderoModelType type, LoaderoModel model) {
        ClientUtils.checkArgumentsForNull(url, type, model);
    
        LoaderoModel result = null;
        String modelToJson = ClientUtils.modelToJson(model);
        String methodName = "";
        try {
            HttpEntity entity = new StringEntity(modelToJson);
            ...
            if (res.getStatusLine().getStatusCode() == HttpStatus.SC_CREATED) {
                result = ClientUtils.httpEntityToModel(res.getEntity(), type);
